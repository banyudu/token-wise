...
pub struct AppEntitlement {
    ...
    pub purchase_type: Option<String>, // "annual" | "lifetime"
...
pub async fn get_app_entitlement() -> AppEntitlement {
    ...
    if let Ok(ent) = storekit::check_entitlements() {
        if ent.has_lifetime.unwrap_or(false) {
            return AppEntitlement {
                state: "paid".to_string(),
                trial_days_remaining: 0.0,
                purchase_type: Some("lifetime".to_string()),
                annual_expires: None,
            };
        }
        if ent.has_annual.unwrap_or(false) {
            return AppEntitlement {
                state: "paid".to_string(),
                trial_days_remaining: 0.0,
                purchase_type: Some("annual".to_string()),
                annual_expires: ent.annual_expires,
            };
        }
    }
